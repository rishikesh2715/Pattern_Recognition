...
plt.ion()
# gaussian kernel
def rbf_kernel(X1, X2, sigma=1.75):
    gamma = 1/(2 * sigma**2)
    sq_dist = np.sum(X1**2, 1).reshape(-1, 1) + np.sum(X2**2, 1) - 2 * np.dot(X1, X2.T)
    return np.exp(-gamma * sq_dist)

# predict
def predict(X, sv_X, sv_y, sv_lambdas, b, sigma=1.75):
    K = rbf_kernel(X, sv_X, sigma)
    return np.dot(K, sv_lambdas * sv_y) + b
...
for C in C_values:
    ...
    plt.contour(xx, yy, Z, colors='k', levels=[-1, 0, 1], alpha=1, linestyles=['--', '-', '--'])
